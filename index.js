...
(async () => {
  ...
  const links = await page.evaluate(() =>
    [...document.querySelectorAll(".st-text a")].map((e) => e.href)
  );
  ...
  for (let i = 0; i < links.slice(0, 2).length; i++) {
    ...
    const productLink = await newPage.evaluate(() => {
      const res = [...document.querySelectorAll(".makers li a")].map(
        (e) => e.href
      );
      return res;
    });
  ...
  for (let i = 0; i < productLinks.length; i++) {
    const newPage = await browser.newPage();
    await newPage.goto(productLinks[i], { waitUntil: "domcontentloaded" });
    await wait(2);
      ...
      const data = {
        ...
        network: {
          ...
          technology: document.querySelector("[data-spec='technologynettech']")
            ? document.querySelector("[data-spec='technologynettech']")
                .textContent
            : null,
        ...
        display: {
          ...
          resolution: document.querySelector("[data-spec='displayresolution']")
            ? document.querySelector("[data-spec='displayresolution']")
                .textContent
            : null,
        },
        ...
        battery: {
          type: document.querySelector("[data-spec='batdescription1']")
            ? document.querySelector("[data-spec='batdescription1']")
                .textContent
            : null,
